...
/**
 * Creates a GitHub webhook API endpoint handler for revalidating pages.
 *
 * @remarks
 * Note: The implementation in Fumadocs had issues during `next build`
 * due to a barrel export that included the `initHotReload` module alongside.
 * ```
 * ERROR: `ssr: false` is not allowed with `next/dynamic` in Server Components.
 * ```
 * The `createGithubWebhookAPI` itself did not have issue and has been
 * kept as is here.
 *
 * @public
 */
export function createGithubWebhookAPI({
	...
	return {
		// biome-ignore lint/complexity/noExcessiveCognitiveComplexity: Fumadocs Implementation
		async POST(request) {
